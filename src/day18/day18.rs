...
fn split_line(cline: ((i64,i64),(i64,i64)), nline:((i64,i64),(i64,i64))) -> Vec<((i64,i64),(i64,i64))>{
...
pub fn run(){
    let input = include_str!("input").lines().map(|l| l.split(" ").collect::<Vec<&str>>());
    ...
    for instruction in input{
        ...
        let distance = usize::from_str_radix(&instruction[..instruction.len()-1], 16).unwrap() as i64;

        let new_hole = (current_hole.0 + dir.0 * distance, current_hole.1 + dir.1 * distance);
    ...
    while lines.len() > 0{
        ...
        while line_queue.len() > 0{
            ...
            if !handled{
                next_current_lines.push(((next_lines[0].0.0, cline.0.1),(next_lines[0].0.0, cline.1.1)));
            }
        ...
        for line in cut_lines{
            ...
            for corner in [line.0, line.1]{
                if next_current_lines.clone().iter().any(|nl| nl.0 == corner || nl.1 == corner){
                    terminal_squares -= 1;
                }
            }
