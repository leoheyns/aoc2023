    ...
    let part1: usize = include_str!("input")
        ...
        .map(|l| {
            l.split(":").collect::<Vec<&str>>()[1]
                ...
                .all(|pull| {
                    pull.split(",")
                        .map(|num_color| num_color.split(" ").skip(1).collect::<Vec<&str>>())
                        .all(|pair| match (pair[0].parse::<usize>().unwrap(), pair[1]) {
                            (1..=12, "red") => true,
                            (1..=13, "green") => true,
                            (1..=14, "blue") => true,
                            _ => false,
        ...
        .map(|(game_number, _)| game_number + 1)
        .sum();
    ...
    let part2: usize = include_str!("input")
        ...
        .map(|l| {
            l.split(":").collect::<Vec<&str>>()[1]
                ...
                .fold([0, 0, 0], |current, pair| {
                    match (pair[0].parse::<usize>().unwrap(), pair[1]) {
