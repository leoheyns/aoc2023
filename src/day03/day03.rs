...
pub fn run() {
    let lines: Vec<Vec<char>> = include_str!("input")
        .lines()
        ...
        for j in 0..lines[0].len() {
            ...
            for k in 0..=2 {
                for l in 0..=2 {
                    ...
                    if check_y < (lines.len() as i32)
                        && check_y >= 0
                        && check_x < lines[0].len() as i32
                        && check_x >= 0
                        && lines[i][j].is_digit(10)
                    {
                        if lines[i + k - 1][j + l - 1] != '.'
                            && !lines[i + k - 1][j + l - 1].is_digit(10)
                        {
                            is_part_num = true;
                        }
                    }
    ...
    for i in 0..lines.len() {
        if is_part_num && current_num.len() > 0 {
            ...
            for j2 in 0..current_num.len() {
                num_grid[i - 1][lines[0].len() - 1 - j2] =
                    (num_count, current_num.parse::<usize>().unwrap())
            }
        }
                            ...
                            && check_x >= 0
                        {
                            if num_grid[i + k - 1][j + l - 1].0 > 0 {
                                if num_1 == (0, 0) {
                                    ...
                                    && num_1.0 != num_grid[i + k - 1][j + l - 1].0
                                {
